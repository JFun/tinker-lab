...
func _ready() -> void:
	...
	if level_idx + 1 < Items.LEVELS.size():
		...
		# Name the destination by its BUILDING (e.g. "Clock Tower"), matching the
		# village hub's labels — NOT the abstract level theme ("Frame & Motion"),
		# which confused players who'd just seen the building name on the homepage.
		var next_npc = Npcs.by_level(level_idx + 1)
		var next_name: String = next_npc.building_label if next_npc != null \
			else String(next_lvl.name)
		# Primary CTA — the hero. A SINGLE line "Level N — <Building>": no "Next"
		# prefix (the gold styling + subdued "Not now" button below already mark
		# this as the go-forward action) and no trailing arrow. Warm gold-bordered
		# fill so it clearly out-ranks the secondary button.
		var go_next := Button.new()
		go_next.text = "Level %d — %s" % [level_idx + 2, next_name]
		go_next.add_theme_font_size_override("font_size", 22)
		go_next.add_theme_color_override("font_color", Color(1.0, 0.92, 0.72))
		go_next.custom_minimum_size = Vector2(0, 72)
